use halo2_proofs::{
    ...
    transcript::{Keccak256Read, Keccak256Write, TranscriptReadBuffer, TranscriptWriterBuffer, EncodedChallenge},
};
use snark_verifier::{
...
pub fn prove_ast<T: FieldElement>(
    ...
    println!("{expanded_row_count_log}");

    println!("Generating parameters...");
    let start = Instant::now();
    let params = ParamsKZG::<Bn256>::new(22);
    let params_app = {
        let mut params = params.clone();
        params.downsize(expanded_row_count_log);
        params
    };
    let duration = start.elapsed();
    println!("Time taken to generate parameters: {:?}", duration);

    //let params = ParamsKZG::<Bn256>::new(expanded_row_count_log);
    let vk = keygen_vk(&params_app, &circuit).unwrap();
    ...
    .unwrap();
    let duration = start.elapsed();
    println!("Time taken to generate first snark: {:?}", duration);

    let proof = transcript.finalize();

    let mut transcript = Keccak256Read::init(&proof[..]);

    println!("Verifying first snark...");
    let start = Instant::now();
    assert!(verify_proof::<_, VerifierGWC<_>, _, _, _>(
        &params_app,
        &vk,
        SingleStrategy::new(&params_app),
        &[&inputs],
        &mut transcript
    )
    .is_ok());
    let duration = start.elapsed();
    println!("Time taken to verify first snark: {:?}", duration);

    // Compression \/

    let num_instance = vec![];
    println!("Generating circuit and pk for compression snark...");
    let start = Instant::now();
    ...
    */
    ...
    println!("Proving compression snark...");
    let start = Instant::now();
    ...
    let duration = start.elapsed();
    println!("Time taken to generate compressed snark: {:?}", duration);
    ...
    let plonk_proof = PlonkVerifier::read_proof(&vk, &protocol, &instances, &mut transcript).unwrap();
    PlonkVerifier::verify(&vk, &protocol, &instances, &plonk_proof).unwrap();

    println!("{}", &loader.yul_code());
